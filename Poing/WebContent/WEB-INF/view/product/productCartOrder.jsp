...
<%
	List<ProductDTO> opt = (List<ProductDTO>) request.getAttribute("opt");
	Iterator<ProductDTO> ir = opt.iterator();
	int sum = 0;
	while (ir.hasNext()) {
    ...
    <span>03 결제완료</span>
				...
				<div class="pay order">
							...
							<c:forEach items="${list}" var="dto">
								<tr data-id="1299328">
									<td class="info"><a class="image"
										href="/product/detail/5904" target="_blank"> <i
											class="image border_radius medium"
											style="background-image: url(${dto.tic_img});"></i>
									</a> <a class="name" href="/product/detail/5904" target="_blank">${dto.rest_name }</a>
										<div class="valid_date">
											유효기간: <span>${dto.tic_validate_content }</span>
										</div>
									
										<ul class="options">
											<li data-id="19044" data-limit="4"><c:forEach items="${opt }" var="op">
											<c:if test="${dto.tic_cart_seq eq op.tic_cart_seq }">
													<div class="name">${op.tic_op_name }</div>
													<div class="price">${op.tic_dc_price }원</div>
													<div class="count">${op.tic_op_purchas_cnt }</div>
								
													<div class="total_price">
														<span>${op.tic_dc_price * op.tic_op_purchas_cnt }</span>원
													</div>
													</c:if>
											</c:forEach></li>
											<li class="total">티켓금액 <span class="jindong"><%= sum %>원</span>
											</li>
										</ul>
										</td>
									<td class="reserve">
										<div class="date">
											날짜: <span>${dto.tic_reserve_date }</span>
										</div> <!-- <div class="time">시간: <span>오후 6:00</span></div> -->
										<div class="count">
											인원: <span>${dto.tic_num_of_people }</span>
										</div> <br>
									<div class="comment">
											요청사항: <span>${dto.tic_request }</span>
										</div>
									</td>
								</tr>
								</c:forEach>
						...
						<table>
							<thead>
								<tr>
									...
									<th class="point">포인트 (보유 포인트: <span>${authUser.m_point }</span>P)
									</th>
							...
							<tbody>
								<tr>
									...
									<td class="point"><i class="icon subtract"></i> <input
										id="point" type="text" value="0" data-max="1000000000">P
									</td>
									<td class="total"><span><%=sum %></span>원</td>
								</tr>
							</tbody>
						</table>
					</div>
					...
					<div class="section type card-area empty ">
						...
						<div class="paybox">
							<div class="title">
								<input type="radio" id="poing-pay" value="poing_pay" name="pay"
									checked=""><label for="poing-pay"
									style="margin-top: 10px;"><i class="icon poingpay"></i></label>
								<span class="tip">신한 Tasty 카드 청구시 10% 추가 할인</span>
								<button class="new_card" type="button" tabindex="-1">
							...
							<div class="body">
								<div class="box full">
									...
									<div class="info">
										<span>카드를 등록해주세요.</span> <span class="notice">지금 등록하시면
											3,000P 지급!</span>
									</div>
								</div>
							</div>
						...
						<div class="body">
							<div class="tab">
								...
								<div class="content">
									<div class="selected">
										<p class="policy">
											3일 전 취소 : 취소수수료 없음<br> 2일 전 취소 : 20%의 취소수수료 발생<br>
											1일 전 취소 : 30%의 취소수수료 발생<br> 당일 취소 : 환불 불가
										</p>
									</div>
							...
							<div class="agreement">
								<input id="ag1" class="select" type="checkbox"> <label
									for="ag1">취소수수료 규정 동의</label> <input id="ag2" class="select"
									type="checkbox"> <label for="ag2">상품 사용 및 환불 규정
									동의</label>
							</div>
						</div>
					</div>
