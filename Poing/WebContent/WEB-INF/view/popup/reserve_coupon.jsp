...
<div id="reserve_coupon">
	...
	<div class="buttons">
					<button type="button" class="accept" data-id="1297740" data-close>예약 접수</button>
			</div>
</div>
...
$(function(){
	// 수량 컨트롤
	$("#reserve_coupon .box.count>div>button").click(function(){
		...
		if( $(this).hasClass('increase') ) {
			if(value < max)
				...
				noticePopupInit({message:'예약 가능한 최대 인원입니다.'});
		} else if( $(this).hasClass('decrease') && value > min )
			target.val( --value );

		$("#reserve_coupon>.body>.request>.summary>.count").text(value +"명");
	});
	$("#reserve_coupon .box").click(function() { 
		$(this).find(">div>div").toggle(); 
	});
	// 요청사항 30자 제한
	$("#reserve_coupon .section.comment>textarea").bind("input", function(){
		if($(this).val().length > 30) 
			$(this).val( $(this).data('input') );

		$(this).data('input', $(this).val());

		$("#reserve_coupon .text_count>span").text( $(this).val().length );
	});

	// 타임테이블
	$("#reserve_coupon div.timetable>ul>li").click(function(e) { 
		e.stopPropagation();

		var time = $(this).data('time');
		$(this).closest(".timetable").siblings("input").val( time );
		$(this).closest(".timetable").siblings("span").text( $(this).text() );

		$("#reserve_coupon>.body>.request>.summary>.time").text( $(this).text() );
	...
	range.setDate(0);
	var holidays = $.parseJSON('["2019.03.18","2019.03.19","2019.03.20","2019.03.22"]');
	for(var i=0; i<holidays.length; ++i)
		holidays[i] = new Date(holidays[i].replace(/\./g, '-'));

	var $datepicker = $('#reserve_coupon div.calendar').datepicker({
        dateFormat: 'yy-mm-dd',
		inline: true,
		firstDay: 00,
		...
		{
			if( $("#reserve_coupon div.timetable>ul[data-day="+day.getDay()+"]>li").length == 0 )
				return [false, ""];
			...
			for(var i=0; i<holidays.length; ++i)
			{
				...
				if(day.getYear() === holiday.getYear() && day.getMonth() === holiday.getMonth() && day.getDate() === holiday.getDate())
					return [false, ""];
			}
		...
		onSelect: function(date, obj){
			...
			var lead_hour = 60 * 2;
			var available_time = (now.getHours()*60) + now.getMinutes() + lead_hour;
			var table = $("#reserve_coupon div.timetable>ul").removeClass('selected')
							.filter("[data-day="+day+"]").addClass('selected')
							.find(">li").show();

			if(now.getMonth() === selected.getMonth() && now.getDate() === selected.getDate()) {
				...
				if( i === table.length) {
					$("#reserve_coupon div.timetable>ul.disable").addClass('selected')
						.siblings().removeClass('selected');
				}
			}
			$("#reserve_coupon .box.date>div>input").val(date);
			$("#reserve_coupon .box.date>div>span").text(selected.getFullYear()+"." + (selected.getMonth()+1)+"." + selected.getDate());

			if($("#reserve_coupon div.timetable>ul.selected>li").length > 0)
				$("#reserve_coupon div.timetable>ul.selected>li:not([style*='none']):first").click();
			else {
				$("#reserve_coupon .box.time>div>input").val('');
				$("#reserve_coupon .box.time>div>span").text('');
				$("#reserve_coupon>.body>.request>.summary>.time").text('');
			}
			$("#reserve_coupon div.calendar").hide();

			$("#reserve_coupon>.body>.request>.summary>.date").text( (selected.getFullYear() % 100)+"년 " + (selected.getMonth()+1)+"월 "+selected.getDate()+"일 (" + ['일','월','화','수','목','금','토'][selected.getDay()] + ")");
		}
	}).click(function(){ 
		return false; 
	});
    ...
    $("#reserve_coupon div.calendar .ui-datepicker-calendar td:not(.ui-state-disabled):first").click();
	...
	function reservation(e)
	{
		...
		if($("#reserve_coupon .phone").length > 0) {
			noticePopupInit({message:"번호 인증을 해주세요!"});
            return false;
            ...
            var mode = $("#mode").val();
            var request = mode ? '/pay/modifyCartReservation':'/restaurant/ajaxEditreserve';
			...
			$.ajax({
				...
				'success': function(res) {
                    ...
                    } else $.popup("confirm", {'text': '예약 접수에 실패했습니다.', 'alert':true});
				}
			});	
		}
	}
